...
public class ExceptionsHandler {
    ...
    @ExceptionHandler(value = { TopicsCategoryServiceException.class })
    public ResponseEntity<Object> handlerTopicsCategoryServiceException(TopicsCategoryServiceException ex ) {
        return new ResponseEntity<>
                (new ResponseDto<>(false, ex.getMessage()), new HttpHeaders(), detectCodeStatus(ex.getMessage()));
    }
    ...
    private static HttpStatus detectCodeStatus(String message) {
        String code = message.replaceAll("[^0-9]", "").trim();

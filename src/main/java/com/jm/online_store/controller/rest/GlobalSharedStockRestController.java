...
public class GlobalSharedStockRestController {
    ...
    public ResponseEntity<ResponseDto<SharedStock>> addSharedStock(@RequestBody SharedStock sharedStock) {
        sharedStock.setUser(userService.getCurrentLoggedInUser());
        sharedStockService.addSharedStock(sharedStock);
        return new ResponseEntity<>(new ResponseDto<>(
                true, sharedStockService.addSharedStock(sharedStock)), HttpStatus.OK);

...
public class ManagerTopicsCategoryRestController {
     ...
     * @return ResponseEntity<TopicsCategory> возвращает единственную категорию тем со статусом
     *     ответа, если категория тем с таким id не существует - только статус
     */
    @GetMapping("/{id}")
     ...
     */
    @PostMapping
    public ResponseEntity<TopicsCategory> createTopicsCategory(
     ...
     */
    @PutMapping("/{id}")
    public ResponseEntity<TopicsCategory> updateTopicsCategory(
        @PathVariable(name = "id") long id, @RequestBody TopicsCategory topicsCategory) {
      if (topicsCategoryService.existsByCategoryName(topicsCategory.getCategoryName())) {
        return ResponseEntity.status(HttpStatus.NOT_MODIFIED).build();
      }
      if (!topicsCategoryService.existsById(id)) {
     ...
     *
     * @param id идентификатор категории
     * @return ResponseEntity<TopicsCategory> возвращает заархивированную категорию тем со статусом
     *     ответа, если категория тем с таким id не существует - только статус
     */
    ...
    public ResponseEntity<TopicsCategory> unarchiveTopicsCategory(
